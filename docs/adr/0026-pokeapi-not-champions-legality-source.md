...
SoT を catalog YAML（hand-authored）へ移した路線の延長で、技メタの SoT も catalog へ寄せられる。
...
- **技メタ（type / damageClass / power / accuracy / pp / priority）の SoT を `data/champions/catalog/moves.yaml`
  （hand-authored・コミット）へ移す。** `generate.ts` は技メタについて `data/raw` を読まず catalog YAML を
...
- **良い点**:
  ...
  - 技の出自・技メタが Serebii 第一優先の hand-authored 入力から直読・直編集でき、Champions 独自の
...
- **悪い点 / コスト**:
  - 技メタが hand-authored になり、新規技の追加時に type / power 等を人手で正しく記入する責任が生じる
...
- **トレードオフ / 留意点**:
  - **reversible**: PokeAPI が将来 Champions に正式対応したら本決定を見直せる。その際は本 ADR を supersede し、
    learnset 照合の復活・技メタ source の再検討を行う（hand-authored catalog は SoT として残しつつ機械照合を
    補助に戻す等）。
