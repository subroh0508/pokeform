...
# 0009. dev / CI 用に Docker 化する

## Context

コーディングエージェント・ローカル・CI が**同一環境**で `pnpm verify` を回せないと「自分の環境では緑」問題が起き、検証ゲート（[ADR 0013](./0013-git-hooks-over-claude-hooks.md)）の信頼性が損なわれる。ランタイム（[ADR 0004](./0004-nodejs-runtime.md)）/ パッケージ管理（[ADR 0005](./0005-pnpm-package-manager.md)）の差で結果が変わるのを防ぎ、再現性を確保したい。具体バージョンには言及しない（版の SoT は [ADR 0008](./0008-toolchain-version-source-of-truth.md)）。
