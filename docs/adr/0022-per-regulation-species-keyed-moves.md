...
- **generate の責務縮小（段階的）**: per-reg 種族 dex の `moves` は YAML 値を採用する（`catalog ∩ learnset` の
  その場導出を廃止）。**本決定の当初は覚えない技の検出（YAML moves ⊆ learnset）を generate に残す**が、
  後続で `check:regulation`（authoring 時ゲート）へ移設し、`generate.ts` を完全な変換専任にする。
...
- **トレードオフ / 留意点**:
  - 当初は検証が generate に残る（安全性のため）。完全な変換専任化と `check:regulation` は後続で行う。
