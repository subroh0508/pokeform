...
- **Phase 6 / 7 は後から挿入した 2 skill 分割 phase**: `data/` 完全削除からの復元を「静的データ著述（`author-static-data` skill・Phase 6）」と「reg 取得オーケストレーション（`author-regulation-data` skill・Phase 7・`showdown-sync.yml` は据え置き）」に分離するため、本投入の手前へ挿入した（旧 full-rollout Phase 6 = 本投入は Phase 8 へ renumber・参照追従は [[planning]] の insert / renumber チェックリストに従う）。
...
- **Phase 8（本投入）は >1000 行を 1 PR 許容**: 全種・全 movepool 規模で意味ある粒度分割が困難なため（[[planning]] 6 基準⑤ の例外・[`OVERVIEW.md`](./OVERVIEW.md#phase-分割6-基準の評価サマリ) に根拠）。skill 手順（Phase 6/7）はデータ投入（Phase 8）から分離済み。
