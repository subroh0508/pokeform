...
flowchart TD
    ...
    P5 --> P6[phase-06 — 全件名辞書 author-static-data + PokeAPI カタログ workflow + generate superset 緩和 + ADR]
    P6 --> P7[phase-07 — reg 取得 skill author-regulation-data 新設]
    P7 --> P8[phase-08 — M-A・M-B 全データセット本投入（plan 09 Phase 4 移植）]
```
...
- [ ] [Phase 6 — 全件名辞書 author-static-data（update-catalog リネーム）+ PokeAPI カタログ workflow](./phase-06-author-static-data.md)
- [ ] [Phase 7 — レギュレーション取得の author-regulation-data skill 新設](./phase-07-author-regulation-data-skill.md)
- [ ] [Phase 8 — M-A・M-B 全データセット本投入（plan 09 Phase 4 の cross-plan move）](./phase-08-full-rollout.md)
...
- **Phase 6 / 7 は後から挿入した 2 skill 分割 phase**: `data/` 完全削除からの復元を「reg 非依存の全件名辞書（`author-static-data`・Phase 6・`update-catalog` リネーム）」と「reg 依存の解禁取得（`author-regulation-data`・Phase 7・`showdown-sync.yml` は据え置き）」に分離するため、本投入の手前へ挿入した（旧 full-rollout Phase 6 = 本投入は Phase 8 へ renumber・参照追従は [[planning]] の insert / renumber チェックリストに従う）。`rules.yaml`・`type-specs.yaml` はどちらでも触らない静的コミット（変更頻度極小・`data/` 完全削除時のみ手作業復元）。
...
- **Phase 8 は plan 09 Phase 4 の移植**: 旧 [09-champions-data-rollout](../completed/09-champions-data-rollout/README.md) の最終フェーズ（全データセット本投入）を本計画群へ cross-plan move し、新パイプライン（`author-regulation-data` 実行 + verify-showdown-pr 照合）へ改訂したもの。移動・参照追従は [[planning]] の cross-plan move チェックリストに従う。
- **Phase 8（本投入）は >1000 行を 1 PR 許容**: 全種・全 movepool 規模で意味ある粒度分割が困難なため（[[planning]] 6 基準⑤ の例外・[`OVERVIEW.md`](./OVERVIEW.md#phase-分割6-基準の評価サマリ) に根拠）。skill / src 手順（Phase 6/7）はデータ投入（Phase 8）から分離済み。
