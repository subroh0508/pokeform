...
Phase 1-7 で揃った新パイプライン（showdown 抽出 + 転記 + ja backfill + GitHub Actions + 照合スキル + 静的著述 skill + reg 取得 skill）を使い、M-A・M-B の**全解禁情報（全種族・全使用可能技・全解禁持ち物・全メガ）**を **`author-regulation-data` skill を各レギュに対して実行**して投入し、両レギュを完成させる（skill の dogfood でもある）。

- スコープ内: `author-regulation-data` skill を M-A → M-B の順で実行して本投入、`verify-showdown-pr` で Serebii 照合、`pokemon-data-reviewer` レビュー。
- スコープ外: M-C 以降のレギュレーション。新機能・新 rule。スキーマ / generate / skill の再設計（確定済み）。skill 手順そのものの新設（Phase 7 で確定）。
...
- **Phase 7 完了**: `author-regulation-data` skill（reg ごとの取得オーケストレーション = reset → `showdown-sync.yml` dispatch → ja gap 補完 → 照合 → per-reg 静的著述・[phase-07](./phase-07-author-regulation-data-skill.md)）が新設されている。
- 確定済み rule: [[data-pipeline]] / [[cli-and-io]] / [[type-conventions]] / [[testing]]。ADR 0039 / 0040。

## タスク

- [ ] **M-A 全量投入**: `author-regulation-data m-a` を実行（skill が author-static-data 欠落チェック → per-reg reset → `showdown-sync.yml` dispatch → ja gap 補完 → `verify-showdown-pr` 照合 → per-reg 静的著述 を駆動）→ authoritative PR。
...
- **データ投入 PR（>1000 行）を 1 PR 許容**: 全種・全 movepool 規模で意味ある粒度分割が困難なため各レギュ 1 PR とする（[[planning]] 6 基準⑤ の例外・OVERVIEW に根拠）。skill 手順（harness 資産）は Phase 7 で分離済みゆえ、本 phase の PR はデータのみ（harness 資産を混ぜない）。レビュー容易性のため specs / per-reg YAML の差分と生成物差分を分けて説明する。
