...
> `author-static-data`（Phase 6・静的）と対になる **per-reg 取得オーケストレーション skill** を新設する。`data/` 完全削除からの復元を「静的著述（Phase 6）」と「reg 取得（本 phase）」に分離する 2 skill 分割方針の後段。既存の `showdown-sync.yml` workflow は**据え置き**、本 skill がそれを dispatch する取得層として利用する（skill は手順オーケストレーション・workflow は取得実体という役割分担）。

## 目的 / スコープ

指定レギュレーション 1 つ分の解禁データを、`data/` 完全削除・部分欠損・完成済みのいずれの状態からでも同じ手順で収束させる**冪等な取得 skill** `author-regulation-data <reg>` を新設する。skill は取得実体を再実装せず、確定済みの機械経路（`showdown-sync.yml` / `fetch:ja-names` / `serebii:*` / `verify-showdown-pr` / `check:regulation` / `generate:data`）を**オーケストレーション**する。

- スコープ内: skill 新設（canonical + symlink）、前提ゲート（`author-static-data` 欠落チェックの fail-fast 誘導）、per-reg reset → `showdown-sync.yml` dispatch → ja gap 補完（PokeAPI → Serebii 速報 → 手入力）→ `verify-showdown-pr` 照合 → per-reg 静的著述（`<reg>/index.yaml` の period・`languages/regulations.yaml` エントリ）→ `pokemon-data-reviewer` 依頼、までの手順化。冪等スキップ（存在レコードは append/既存尊重で触れない）の明文化。
- スコープ外: **M-A・M-B の実データ投入そのもの（Phase 8 が本 skill を実行）**。`showdown-sync.yml` workflow の改修（据え置き・本 skill は dispatch するのみ）。`author-static-data`（Phase 6）。`showdown:types` 抽出の新設（OVERVIEW スコープ外の維持）。M-C 以降のレギュレーション。
...
- [ ] **前提ゲート**: 取得開始前に `author-static-data` の欠落チェックを通す **fail-fast 誘導**を skill 本文に記す（静的層が欠けていれば先に author-static-data を実行させる。skill 間の自動連鎖はしない = 明示的オーケストレーションに委ねる）。
- [ ] **取得手順のオーケストレーション**（各段は既存経路へ委譲・再実装しない）:
  ...
  - ja gap 検出（`generate:data` の `requireNames` エラーで欠落 id を特定）→ `fetch:ja-names`/`sync:ja-names`（PokeAPI）→ 残余は `serebii:<dataset>`（速報 ja）→ なお残れば手入力。Champions 固有メガ / メガストーンの ja 欠落が典型（[[data-pipeline]]）。
...
- [ ] [[data-pipeline]] に本 skill を per-reg 取得の著述経路として指すポインタを追記する（`author-static-data`（静的）/ `author-regulation-data`（reg 取得）/ `update-catalog`（ja 補完）の経路別 skill 群として一貫させる）。
...
- `author-regulation-data` skill が新設され、前提ゲート / reset / dispatch / ja gap 補完 / 照合 / per-reg 静的著述 / 冪等スキップ が本文に定式化されている。
...
- **skill と投入の分離**: 本 phase は harness 資産（skill + rule 追記）のみ、実データ投入は Phase 8。Phase 6/7 で skill を確定し Phase 8 でデータ投入する構造で、harness-review 対象（skill）と pokemon-data-reviewer 対象（データ）の PR を分離しレビュー性を保つ。
