...
> 全量本投入（Phase 9）の手前に**挿入**した phase。`data/` 完全削除状態からの復元を「**静的データの著述**（本 phase）」「**全件名辞書**（`update-catalog`・Phase 7）」「**reg 取得**（`author-regulation-data`・Phase 8）」に分離する方針の前段。挿入に伴う renumber（旧 full-rollout Phase 6 → 9）の参照追従は [[planning]] の insert / renumber チェックリストに従う。
...
- スコープ内: skill 新設（canonical + symlink）、欠落チェック手順、導出著述手順、[[data-pipeline]] への著述経路ポインタ追記、`author-regulation-data`（Phase 8）との責務境界の明文化。
- スコープ外: レギュレーション情報の取得（showdown 5 データセット / PokeAPI ja / Serebii 速報 = `author-regulation-data` skill・Phase 8 の責務）。per-reg 静的データ（`<reg>/index.yaml` の period・`languages/regulations.yaml` のエントリ = reg 依存ゆえ Phase 8 側）。`type-specs` の showdown 抽出自動化（`showdown:types` 新設 = OVERVIEW スコープ外の維持・将来計画へ送り）。`data/` 全削除の実行そのもの（Phase 9）。
...
- [ ] **前提ゲートの明文化**: reg 取得（`author-regulation-data`・Phase 8）を実行する前に本 skill の欠落チェックを通す **fail-fast 誘導**を skill 本文に記す（skill 間の自動連鎖はしない・束ねる場合は明示的オーケストレーションの責務。author-regulation-data 側からも本ゲートを呼ぶ）。
...
- **境界はファイル単位でなくデータ単位**: `languages/regulations.yaml` はファイルとしては reg 非依存だがエントリが per-reg のため本 skill は触らない（`author-regulation-data`・Phase 8 の責務）。`languages/*.yaml` のファイル骨格（空 map）scaffold は全件辞書化（`update-catalog`・Phase 7）が担う。
