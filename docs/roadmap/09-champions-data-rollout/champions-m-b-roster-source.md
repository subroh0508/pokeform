...
投入対象外**とした（Phase 1 で Mega Starmie / Mega Dragonite を外したのと同一判断・PokeAPI vendor に mega slug が
無く `fetch:data` が 404 → `materialize` が構造データを埋められない）:
...
- **投入対象外（Champions 固有）**: Mega Staraptor / Scolipede / Scrafty / Eelektross / Pyroar / Malamar /
  Barbaracle / Dragalge / Falinks（および Mega Raichu X/Y）。これらは固有特性（eelevate / fire-mane 等）も持ち、
  そのストーン（staraptite / scolipite / … / raichunite-x/y）も PokeAPI 非存在。
- per-reg `m-b/mega.yaml` = **10 種**（M-A 由来 5: charizard X/Y・gengar・garchomp・tyranitar・lucario + 新規 5）。

> **Phase 3 申し送り**: Champions 固有メガ / アイテムの構造データを Serebii scrape 由来で著述する経路を
> skill / materialize に補えば、Mega Staraptor 系・Mega Starmie / Dragonite を本投入（Phase 4）で起こせる。
