...
直列チェーン: 先行する [04-generated-layout-redesign](../completed/04-generated-layout-redesign/README.md)（Phase 1-3 再編）→ [05-move-master-scraper-refactor](../completed/05-move-master-scraper-refactor/README.md)（技マスター取得 + 役割分割 + skill 再編）→ 本計画群（09 Phase 1 → 2 → 3）の**一方通行**。04 / 05 へ戻る依存は無い。全量本投入（旧 09 Phase 4）は [10-showdown-first-data Phase 6](../10-showdown-first-data/phase-06-full-rollout.md) が引き継ぐ。
