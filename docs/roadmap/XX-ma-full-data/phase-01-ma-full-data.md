...
> パイプライン**経由。
>
> **本 phase は [04-generated-layout-redesign の Phase 1-3](../completed/04-generated-layout-redesign/README.md) で再編した新レイアウト**
> （specs / languages / per-reg 4 オブジェクト・`data/champions/m-a/*` / `data/languages/*`）**と、
> [05-move-master-scraper-refactor](../completed/05-move-master-scraper-refactor/README.md) で技マスター専用取得経路により
> Champions 準拠へ是正済みの `move-specs` の上で実施する**（投入時の技メタは Champions 実値で正しい）。
> 以下の本文中の旧レイアウト表記（`catalog/*` / `regulations/champions/m-a.yaml`）は
> [04 OVERVIEW の実装指針](../completed/04-generated-layout-redesign/OVERVIEW.md#実装指針) の新ツリー対応に読み替える。
