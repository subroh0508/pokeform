...
- **実施順は先行計画 [`07-rules-skills-simplify`](../completed/07-rules-skills-simplify/README.md) を先、本計画（08）を後**にする。先に rules / skills の冗長が削られた状態で構造を動かすほうが、参照張り替え・front matter 付与・二重管理解消の対象がスリムになる。両計画は `code-review.md` / `planning.md` 等で一部重なるため 07 → 08 の順で進め後発（本計画）をリベースする（ハードな相互依存はない）。
...
- **rules / skills の純シンプル化（文体圧縮・約 20〜30% 削減）** = 先行計画 [`07-rules-skills-simplify`](../completed/07-rules-skills-simplify/README.md)。本計画は配置再編・front matter 付与と、その機会に生じる **SoT 実体の二重記述解消**（design ↔ rule のリンク化）までを扱い、本文の行数圧縮そのものはしない。
