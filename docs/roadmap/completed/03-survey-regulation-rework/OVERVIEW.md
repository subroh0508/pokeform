...
投入は [`09-champions-data-rollout`](../../09-champions-data-rollout/README.md)** が担い、依存を一方通行（03 → 04 → 05 → 09）に保つ。
...
generated/YAML レイアウトの上で**実行すべきため、後続計画群 [`09-champions-data-rollout`](../../09-champions-data-rollout/README.md) へ移した。
...
5. 取得スキルが 2 分割（catalog 取得 / regulations 取得）され、catalog 更新チェックポイントが機能する（Phase 12）。
   ...
   **M-A 全186種の全量投入は [`09-champions-data-rollout`](../../09-champions-data-rollout/README.md)** で実施する（いずれも本計画群のスコープ外・
   依存は一方通行 03 → 04 → 05 → 09）。
...
[`09-champions-data-rollout`](../../09-champions-data-rollout/README.md)（全種族投入）へ**一方通行（03 → 04 → 05 → 09）**で繋ぐ。
