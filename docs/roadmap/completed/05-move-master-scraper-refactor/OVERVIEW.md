...
避ける。全種族投入（[`09-champions-data-rollout`](../../09-champions-data-rollout/README.md)）は本計画群で整えた取得パイプライン + 正しい
技マスター値の上で行う。
...
- **技マスター専用取得経路は決定論スクレイパー（層1）として実装**。Serebii の技専用ページ（`attackdex-champions/<move>.shtml`）から
  ...
  [ADR 0037](../../../adr/archive/0037-serebii-move-master-dedicated-path.md) として確定した（取得方式を副産物抽出 → 専用取得へ・
...
- **M-A 全186種の全量投入**（[`09-champions-data-rollout`](../../09-champions-data-rollout/README.md) で実施）。`05` で整えた取得パイプライン +
  正しい技マスター値の上で投入する。
...
[`09-champions-data-rollout`](../../09-champions-data-rollout/README.md)（全種族投入）へ**一方通行（04 → 05 → 09）**で繋ぐ。
