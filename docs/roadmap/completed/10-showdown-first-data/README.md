...
flowchart TD
    ...
    P5 --> P6[phase-06 — 全件名辞書の基盤 author-static-data + pokeapi-names workflow + generate 緩和 + ADR]
    P6 --> P7[phase-07 — 全件名辞書の初回データ投入]
    P7 --> P8[phase-08 — reg 取得 skill author-regulation-data 新設]
```

> **本投入（旧 Phase 9 = M-A・M-B 全データセット本投入）は [11-static-restore-hardening](../../11-static-restore-hardening/README.md) Phase 3 へ cross-plan move した**。plan 10 は Phase 1-8（取得元転換・skill 基盤・pipeline 確定）で完了し、実データの全件投入は復元機構堅牢化（plan 11 P1-3）の後に回す判断。
...
- [x] [Phase 6 — 全件名辞書の基盤（author-static-data リネーム + pokeapi-names workflow + generate 緩和 + ADR）](./phase-06-author-static-data.md)
- [x] [Phase 7 — 全件名辞書の初回データ投入（author-static-data 実行）](./phase-07-languages-populate.md)
- [x] [Phase 8 — レギュレーション取得の author-regulation-data skill 新設](./phase-08-author-regulation-data-skill.md)

> 旧 Phase 9（M-A・M-B 全データセット本投入）は [11-static-restore-hardening](../../11-static-restore-hardening/README.md) Phase 3 へ移した（cross-plan move）。
...
- **Phase 6 は取得を GitHub Actions で完結 + generate 検証を限定緩和**: PokeAPI からの取得 + 整形 + 書き込み + PR 作成を新 workflow（`pokeapi-names.yml`）で実行し、取れない分（メガ ja 等）は PR への手作業追加 commit で補う。全件名辞書化に伴い `generate.ts` の `requireNames` を bijection → specs ⊆ languages（orphan 許容）へ緩め、ADR 0035 を refine する新 ADR を起こす（plan 10 の「検証機構不変」原則の限定例外・OVERVIEW 設計方針に根拠）。
- **旧 Phase 9（M-A・M-B 全データセット本投入）は plan 11 Phase 3 へ cross-plan move**: 旧 [09-champions-data-rollout](../09-champions-data-rollout/README.md) Phase 4 → 本計画群 Phase 9 と移ってきた本投入 phase を、[11-static-restore-hardening](../../11-static-restore-hardening/README.md) Phase 3 へさらに移した（復元機構堅牢化の後に実データ投入する判断）。移動・参照追従は [[planning]] の cross-plan move チェックリストに従う。
