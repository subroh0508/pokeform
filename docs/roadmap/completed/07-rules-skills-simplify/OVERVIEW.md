# 07-rules-skills-simplify — rules / skills の純シンプル化 OVERVIEW
...
事前に 15 rules・16 skills を 1 件ずつ精読して洗い出した削減余地を、独立にマージ可能な phase 群へ落とす。

## 背景 / 動機

ハーネス資産は機能としては健全だが、横断的に 5 類型の冗長が蓄積している:
...
- **共通フレーム抽出**: `code-review` / `harness-review` の SKILL.md は手順 1-5 がほぼ同一。共通フレームを `code-review.md` rule に置き、両 skill は差分（各 checklist 参照）に集中させる。auto-merge ゲートも両 references から rule へ一本化。
...
重量級 / 軽量級 / survey-regulation 単独 / 系統別 / 共通フレーム抽出の粒度で分割する。6 基準:
...
Phase 0〜5 は相互にほぼ独立（並行可能）。後続計画 08（ドキュメント構成再編）より先に実施する。
