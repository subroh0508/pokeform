...
全186種の全量投入（Phase 20）の**手前**に置き、構造データを旧スキーマ（raw 直読）で全量投入してからやり直す
...
- スコープ外:
  ...
  - M-A の全量データ投入（**Phase 20**）。本 Phase は**既存 catalog 分の移行 + 仕組み確定**のみ。
...
- **Phase 10（catalog 日英名 authoring + generate 名前変換専任化）完了**。catalog が `id → { ja, en }` 形式で
  名前・タイプ相性を skill-authored SoT 化済み。本 Phase はその構造データ版で、同じ catalog エントリへ
...
- **materialize の上書き事故**: raw → catalog 転記時に skill-authored 値（Champions 実態に合わせた skill/AI 著述）を
...
- **Phase 20 への前提**: 本 Phase 完了後、Phase 20（M-A 全量投入）は全186種の構造データを catalog YAML へ
