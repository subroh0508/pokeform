...
> **本投入（Phase 4）は [10-showdown-first-data](../10-showdown-first-data/OVERVIEW.md) の Phase 9 へ cross-plan move され、さらに [11-static-restore-hardening Phase 3](../../11-static-restore-hardening/phase-03-full-rollout.md) へ再移動した**。取得方式が pokemon-showdown 第一の正へ刷新されたため、旧 `survey-regulation` + Serebii 第一優先での全量投入は plan 10 の新パイプライン（showdown-sync + verify-showdown-pr）が引き継ぐ。以下の本文中の Phase 4・「本投入」記述は移動前の歴史的経緯であり、現行の本投入計画は **plan 11 Phase 3** を参照すること。

## ゴール

レギュレーション M-A・M-B の解禁データ（種族 / 技 / 持ち物 / メガ）を、整備済みの取得パイプライン
（[03-survey-regulation-rework](../03-survey-regulation-rework/README.md) / [05-move-master-scraper-refactor](../05-move-master-scraper-refactor/README.md)）と
新レイアウト（[04-generated-layout-redesign](../04-generated-layout-redesign/README.md)）の上で投入し、両レギュを完成させる。
...
Phase 3 の人間検証ゲートは PR を伴わない検証マイルストーン。NG なら skill 改修 PR を挟んで Phase 1-2 を再実行する。OK 後の全量本投入は [11-static-restore-hardening Phase 3](../../11-static-restore-hardening/phase-03-full-rollout.md) が引き継ぐ（cross-plan move 済み・取得方式刷新のため）。
...
5. （旧 Phase 4 = 全解禁情報の本投入は [11-static-restore-hardening Phase 3](../../11-static-restore-hardening/phase-03-full-rollout.md) へ移動。本計画群の受け入れは Phase 1-3 まで。）
6. Phase 1-3 で `check:regulation` が 0 終了（参照整合 / schema）、`pokemon-data-reviewer` のレビューで重大な不整合（種族値・タイプ・解禁整合・メガ・技マスター）が無い。

## phase 分割（6 基準の評価サマリ）

全量投入を限定セット検証で de-risk するため、**当初は 4 phase に分けた**（本投入 Phase 4 はその後 [11-static-restore-hardening Phase 3](../../11-static-restore-hardening/phase-03-full-rollout.md) へ cross-plan move したため、本計画群は Phase 1-3 に縮小）。データ投入そのものは意味ある粒度での種族分割が困難なため各投入 phase は 1 PR。6 基準:
...
| ~~Phase 4~~ | M-A・M-B 全解禁情報の本投入 → **[11 Phase 3](../../11-static-restore-hardening/phase-03-full-rollout.md) へ cross-plan move** | （plan 10 へ移動） |

直列チェーン: 先行する [04-generated-layout-redesign](../04-generated-layout-redesign/README.md)（Phase 1-3 再編）→ [05-move-master-scraper-refactor](../05-move-master-scraper-refactor/README.md)（技マスター取得 + 役割分割 + skill 再編）→ 本計画群（09 Phase 1 → 2 → 3）の**一方通行**。04 / 05 へ戻る依存は無い。全量本投入（旧 09 Phase 4）は [11-static-restore-hardening Phase 3](../../11-static-restore-hardening/phase-03-full-rollout.md) が引き継ぐ。
