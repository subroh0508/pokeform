...
- スコープ外: コア実装（Phase 2）。ADR 起票（Phase 1）。技仕様値是正・全種族投入（03 Phase 13/14）。
...
- 本 phase 完了で 04 計画群が完了。次は 03-survey-regulation-rework の Phase 13（技仕様の Champions 対応）→ Phase 14（全種族投入）。
