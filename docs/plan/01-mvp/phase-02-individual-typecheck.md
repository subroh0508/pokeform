...
**「tsc のみ検証」**を実装し、育成済み個体・パーティの不正（覚えない技・使えない特性・存在しないフォルム・性格 up=down・能力ポイント 66/32 違反・同種族重複・未解禁混入）を **`tsc --noEmit` で弾く**。アーキ詳細は [`docs/design/`](../../design/README.md)（旧 `architecture.md` を分割昇格）。
