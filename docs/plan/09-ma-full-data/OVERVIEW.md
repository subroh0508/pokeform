# 09-ma-full-data — M-A 全種族投入（新レイアウト + 整理済みパイプライン上での全量投入）OVERVIEW
...
（05）を挟んで分断される構造を避けるため、**全量投入を独立計画群 09 として切り出した**。これにより依存が
**04（レイアウト再編）→ 05（技マスター取得 + 役割分割）→ 09（全量投入）** の一方通行に整理される。
...
本計画群（09 Phase 1 全量投入）の**一方通行（04 → 05 → 09）**。
