...
- **front matter 規約**: `docs/design/` 配下の Markdown は `last_modified`（ISO8601）+ `adr`（関連 ADR 配列）を必須に持つ。`.claude/rules/` 配下の Markdown は既存 `paths` / `description` に加え `adr`（関連 ADR 配列）を持つ（Claude 自動ロードは `paths` のみ参照するため `adr` は独自キーとして許容し、`gen-rules-index.ts` と非干渉）。
...
- [ ] Decision: design 新設 / 命名（design 採用・architecture 見送り）/ roadmap 改名 + completed/ 運用 / **front matter 規約（design: `last_modified` + `adr` / rules: `adr`）** / design↔ADR↔rule↔src の責務境界を能動・断定形で言い切る。front matter スキーマ例を本文に記載する。
...
4. ADR 本文に design / rules の front matter 規約（`last_modified` / `adr` キー）と スキーマ例が記載されている。
