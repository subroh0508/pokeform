...
3. **front matter 規約の導入**: `docs/design/` 配下の Markdown と `.claude/rules/` 配下の Markdown に、関連 ADR をたどれる front matter（`adr` キー、design は加えて `last_modified`）を付与する。
...
- **front matter 規約**（本計画で新設・Phase 0 ADR で確定する）:
  - **`docs/design/` 配下の Markdown**: 必ず front matter を持ち、`last_modified`（ISO8601 形式の日時）と `adr`（関連 ADR の配列）を記載する。
    ...
    ---
    ```
  - **`.claude/rules/` 配下の Markdown**: 既存 front matter（`paths` / `description`）に加えて `adr` キー（関連 ADR の配列）を持つ。Claude の自動ロードは `paths` のみ参照するため `adr` は**独自キーとして許容**する（読み手・ツールが規約の決定根拠 ADR へ即たどれるようにする）。`gen-rules-index.ts` の `paths` 解釈と非干渉であることを確認する。
...
- **ADR を先頭に置く**（配置・front matter 規約は不可逆 = architecturally significant）。design/ 新設・命名・roadmap 改名・completed/ 運用・**front matter 規約（design: `last_modified` + `adr` / rules: `adr`）**・design↔ADR↔rule の責務境界を 1 本の ADR（採番は `adr-new` で機械決定・現行最大 0037 の次）に確定してから着手する。
...
- **M-A 全種族投入**（[`XX-ma-full-data`](../XX-ma-full-data/README.md)）。データ投入は本計画と独立。
...
5. `.claude/rules/` 配下の全 Markdown が front matter に `adr` キーを持ち、`gen-rules-index.ts` の `paths` 解釈と非干渉（rules-index 再生成が成功）。
...
| 6 | 規約 / skill の plan 参照違反・陳腐化是正 + rules への `adr` front matter 付与 | dangling / 表記是正 + 規約適用 |
