# Phase 6 — 規約 / skill の plan 参照違反・陳腐化是正 + rules への adr front matter

## 目的 / スコープ

事前精読で挙がった構造的な参照違反・軽微な陳腐化を是正し、あわせて Phase 0 ADR で定めた rules の `adr` front matter を全 rule に適用する（**文体圧縮ではない** = 先行計画 07 とは別の「正しさの是正・規約適用」）。

- **plan 参照違反**: `code-review.md:27` の「Phase 9」phase doc 参照は adr.md の「可変 plan ファイルを引かない」に違反 → **ADR 0013 へ張り替え**。
- **rules への `adr` front matter 付与**: 全 `.claude/rules/*.md` の front matter に、関連 ADR を列挙する `adr` キーを追加（Phase 0 ADR の規約）。Claude の自動ロードは `paths` のみ参照するため `adr` は独自キーとして許容し、`gen-rules-index.ts` の `paths` 解釈と非干渉であることを確認する。
- **skill の軽微な陳腐化**:
...
- [ ] 全 `.claude/rules/*.md` の front matter に `adr`（関連 ADR 配列）キーを追加し、`pnpm prepare`（`gen-rules-index.ts`）で rules-index が正しく再生成される（`paths` 解釈非干渉）ことを確認。
...
2. 全 `.claude/rules/*.md` が front matter に `adr` キーを持ち、`rules-index.md` の再生成が成功（`paths` 解釈非干渉）。
