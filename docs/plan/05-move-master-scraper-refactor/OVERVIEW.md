...
避ける。全種族投入（[`09-ma-full-data`](../09-ma-full-data/README.md)）は本計画群で整えた取得パイプライン + 正しい
技マスター値の上で行う。
...
- **M-A 全186種の全量投入**（[`09-ma-full-data`](../09-ma-full-data/README.md) で実施）。`05` で整えた取得パイプライン +
  正しい技マスター値の上で投入する。
...
[`09-ma-full-data`](../09-ma-full-data/README.md)（全種族投入）へ**一方通行（04 → 05 → 09）**で繋ぐ。
