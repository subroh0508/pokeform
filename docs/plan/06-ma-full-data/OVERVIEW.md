# 06-ma-full-data — M-A 全種族投入（新レイアウト + 整理済みパイプライン上での全量投入）OVERVIEW

## ゴール

レギュレーション M-A で解禁されている**全186種・全技・全持ち物・全メガ**を全量投入し、M-A を完成させる。投入は
[04-generated-layout-redesign](../04-generated-layout-redesign/README.md) で再編した新レイアウト（specs / languages /
per-reg 4 オブジェクト）と、[05-move-master-scraper-refactor](../05-move-master-scraper-refactor/README.md) で
...
（05）を挟んで分断される構造を避けるため、**全量投入を独立計画群 06 として切り出した**。これにより依存が
**04（レイアウト再編）→ 05（技マスター取得 + 役割分割）→ 06（全量投入）** の一方通行に整理される。
...
直列チェーン: 先行する [04-generated-layout-redesign](../04-generated-layout-redesign/README.md)（Phase 1-3 再編）→
[05-move-master-scraper-refactor](../05-move-master-scraper-refactor/README.md)（技マスター取得 + 役割分割 + skill 再編）→
本計画群（06 Phase 1 全量投入）の**一方通行（04 → 05 → 06）**。
