...
> **クロスエージェント共有**: skill-creator 準拠で `.claude/skills/implementation-workflow/SKILL.md` に canonical（実体）を作成し、`.agents/skills/implementation-workflow` を `../../.claude/skills/implementation-workflow` への symlink にして Codex と共有（symlink 不可環境は copy フォールバック、`cross-agent.md`／Phase 7）。SKILL.md 本体は ≤500 行、10 フェーズの詳細手順は supporting rule に逃がす。
...
- Phase 4 ADR バックフィル一覧に `0011-implementation-workflow-orchestrator` を追記。
...
| 7 | マージ（auto-merge） | レビュー待ち中の main 再進化を `gh pr view --json mergeable,mergeStateStatus` で検出（必要なら rebase）→ `gh pr ready` → **CI 緑 ＋ ブロッキング指摘なしで `gh pr merge --auto --squash`**（Phase 3 の auto-merge ゲートに委譲） | PR auto-merge 予約 / MERGED |
...
- README の一覧・役割分担表・受け入れ基準に Phase 11 が反映され、ADR `0011` が `architecture.md` と二重記述になっていない。
