...
- [ ] `harness-review-checklist.md`: trigger 精度 / クロスエージェント整合（canonical+symlink パリティ）/ SoT 一貫性 / redaction / paths スコープ正しさ / ≤500 行 / 既存 rule 非矛盾 / ゲート二重化チェック。
...
| `.claude/skills/**`, `.agents/skills/**` | harness-review | description=trigger 精度 / ≤500 行 / **canonical+symlink パリティ** / allowed-tools 最小権限 |
| `AGENTS.md`, `CLAUDE.md` | harness-review | SoT 一貫性（AGENTS=指示 SoT / CLAUDE=`@AGENTS.md` 薄アダプタ）/ 32KiB 意識 |
...
- Google eng-practices — What to look for in a code review / The Standard of Code Review。
- AI 生成コードのレビュー観点（仕様忠実性・エッジ/エラー処理・セキュリティ高リスク既定・前提の安全性）。
- IaC / 設定資産のレビュー観点（規約準拠・誤設定/秘匿情報・スコープ）。
