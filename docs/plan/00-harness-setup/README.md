...
採用するツールと、それぞれの ADR は次のとおり。**具体バージョンはここに再掲しない**（[ADR 0017](../../adr/0017-toolchain-version-source-of-truth.md)）。

| 要素 | ツール | ADR |
|---|---|---|
| 言語 | TypeScript | [0012](../../adr/0012-typescript-language.md) |
| ランタイム | Node.js | [0013](../../adr/0013-nodejs-runtime.md) |
| パッケージ管理 | pnpm（corepack 非依存） | [0014](../../adr/0014-pnpm-package-manager.md) |
| テスト | Vitest（+ coverage-v8） | [0015](../../adr/0015-vitest-test-runner.md) |
| Linter/Formatter | Biome | [0016](../../adr/0016-biome-linter.md) |
| Docker 化（dev/CI） | Dockerfile / compose | [0018](../../adr/0018-dockerize-dev-ci.md) |

> **バージョンの唯一の情報源（SoT）** は `package.json`（devDeps）/ `pnpm-lock.yaml` / `.node-version`（[ADR 0017](../../adr/0017-toolchain-version-source-of-truth.md)）。**メジャー含め最新追従**し、Dependabot + `dep-update` skill が `pnpm verify` / CI 緑で取り込む。
