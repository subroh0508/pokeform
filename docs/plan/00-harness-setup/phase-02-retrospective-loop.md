...
**位置づけ（重要）**: 本フェーズは**ループの「仕掛け」を設置**するもの。実際に回り始めるのは MVP 実装で最初の PR が出てから（その時点で全ハーネスが揃っている）。そのため gh 権限（Phase 9）/ ADR（Phase 4）/ 型・カバレッジゲート（Phase 5）への**前方参照**を許容する（設置 ≠ 即時稼働）。

## 前提（依存）

- Phase 1（`docs/` 構造）。本フェーズで `docs/harness/` 配下と専用 rule/skill を新設する。
- 稼働時（実装 PR 以降）に Phase 4（ADR）/ Phase 5（型・カバレッジゲート）/ Phase 9（gh 権限）を利用する（設置時点では未完でも可）。
...
> **クロスエージェント共有**: 両スキルとも **`skill-creator` skill を使って** `.claude/skills/<name>/` に canonical を作成し、`.agents/skills/<name>` を symlink して Codex と共有（`skill-authoring.md`／`cross-agent.md`／Phase 7）。
...
- [ ] branch 命名 `harness/learnings-batch-YYYY-WW` / `harness/<purpose>`（必要なら `branch-naming.md` を Phase 7 rules に追加）。
- [ ] `finish-phase` skill（Phase 8）末尾に「PR merge 後に `pr-retrospective` を起動」促しを組み込む（本フェーズではメモのみ、実装は Phase 8）。
- [ ] ADR 連携: 🚀 Try のアーキ決定は `[adr]` → Phase 4 の `adr-new`。本ループ導入自体を ADR バックフィル（`00NN-kpt-retrospective-loop`）に追加（Phase 4 で起票）。
- [ ] CLAUDE.md にループ要約を追記（Phase 7 で反映）。
