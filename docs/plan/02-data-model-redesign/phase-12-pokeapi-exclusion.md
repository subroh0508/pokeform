...
ソースを追従させる。全186種の全量投入（Phase 20）の**手前**に置き、誤った前提の上で全量を入れてからやり直す
事故を防ぐ。
...
- スコープ外:
  - M-A の全量データ投入そのもの（**Phase 20**）。本 Phase は**前提（決定 + harness 追従）の確定**に徹する。
  - 種族値・タイプ・特性・持ち物 category 等の構造データの取得元（PokeAPI・ADR 0012・**維持**）。これらの SoT を
    catalog YAML へ移す改訂は **Phase 13**（本 Phase は技メタ / legality のみ扱う）。
  - 構造データの catalog 化・`materialize` 新設・`overrides.yaml` 廃止（**Phase 13**）。
  - data ディレクトリの説明 Markdown 追加（**Phase 14**）。
  - 新機能・01-mvp の機能拡張。
...
- **Phase 10（catalog 日英名 authoring）完了**。名前 SoT が catalog YAML へ移行済みで、技メタ（威力等）の
  SoT 移設もこの skill-authored catalog 路線の延長で検討できる。
...
- [ ] **技メタ SoT 移設**: `generate.ts` が技威力等を PokeAPI raw から読まないようにし、技メタの SoT を
      catalog/`moves.yaml`（skill-authored）等へ移す（具体スキーマは ADR で確定）。生成物 `moves.ts` の構造を追従。
...
- **Phase 20 への前提**: 本 Phase 完了後、Phase 20（M-A 全量投入）は「learnset ∩ legal」ではなく **Serebii 第一
  優先の movepool 全量**を正として投入する（learnset 照合ゲートは存在しない）。Phase 20 doc はこの前提に追従済み。
