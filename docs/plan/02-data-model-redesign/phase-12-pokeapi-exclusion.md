...
ソースを追従させる。全186種の全量投入（Phase 16）の**手前**に置き、誤った前提の上で全量を入れてからやり直す
事故を防ぐ。
...
- スコープ外:
  - M-A の全量データ投入そのもの（**Phase 16**）。本 Phase は**前提（決定 + harness 追従）の確定**に徹する。
...
- **Phase 16 への前提**: 本 Phase 完了後、Phase 16（M-A 全量投入）は「learnset ∩ legal」ではなく **Serebii 第一
  優先の movepool 全量**を正として投入する（learnset 照合ゲートは存在しない）。Phase 16 doc はこの前提に追従済み。
