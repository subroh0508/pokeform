...
加えて、**レギュレーション解禁情報の取得・突き合わせ・データ構造への反映を再利用可能な手順 skill に切り出す**
（`skill-creator` で作成）。M-A だけでなく M-B 以降でも同じ作業を繰り返すため、WebSearch による情報源調査
→ 複数ソース突き合わせ → 全リスト doc 化 → catalog/per-reg YAML への materialize を定型化する。本 phase は
その skill を**M-A の 20 匹で初回実走**して検証し、Phase 4（全量投入）と将来のレギュレーションが再利用する。

- スコープ内: **解禁情報取得 skill の作成（`skill-creator`・canonical + symlink）**、情報源の WebSearch・信頼性
  評価（複数ソース突き合わせ）、M-A 全種族リストの doc 化（出典明記）、無作為20匹の選定方法の記録、20匹分の
  catalog 追記 + M-A per-reg allow 投入、skill を使った end-to-end 検証。
...
- [ ] **解禁情報取得 skill を `skill-creator` で作成**（手順 skill・canonical `.claude/skills/<name>/` +
      `.agents/skills/<name>` symlink）。skill の責務:
  - [ ] 引数（レギュレーション id + 解禁条件）を受け、WebSearch で複数の信頼できる情報源を収集し突き合わせる。
  - [ ] 採用情報源・検証日・矛盾の解消根拠を出典付きで全リスト doc にまとめる。
  - [ ] 解禁エンティティ（種族・技・持ち物・メガ）を catalog（append-only）と per-reg YAML（allow）へ反映する手順。
  - [ ] 機械ゲート（型 / カバレッジ / Biome）は再実装せず `verify` / `generate:data` に委譲（[[skill-authoring]]）。
  - [ ] skill 名は実装時に確定（例: `survey-regulation` 等。kebab-case 動詞 + 対象）。
- [ ] WebSearch で M-A 解禁情報の情報源を複数収集し信頼性を評価（例: 公式 / serebii / bulbapedia / game8 等を
      突き合わせ、矛盾を洗う）。採用した情報源と検証日を記録。**この調査は作成した skill の初回実走で行う**。
...
- **解禁情報取得 skill を新規作成**（`skill-creator` 利用・canonical 実体 + `.agents/skills` symlink でクロス
  エージェント共有）。レギュレーション解禁情報の WebSearch 調査 → 突き合わせ → doc 化 → catalog/per-reg YAML
  反映を定型化し、Phase 4 と将来のレギュレーション（M-B 以降）が再利用する。機械ゲート / レビュー観点は
  再実装しない（[[skill-authoring]] / [[cross-agent]]）。
- 必要なら [[champions-regulation-data-placeholder]] メモリを実データ確定に合わせて更新。情報源 doc は計画群配下に置く。

## 受け入れ基準

- `pnpm verify`（型 / カバレッジ100% / Biome）が緑。
- 解禁情報取得 skill が作成され（canonical + symlink パリティ）、M-A の初回実走で動作確認されている。
...
- **skill の検証可能性**: WebSearch を伴う skill は出力が非決定的になりやすい。skill には「複数ソース突き合わせ・
  矛盾を doc に残す・採用根拠を明記」を手順として組み込み、結果の再現性より**判断過程の追跡可能性**を担保する。
- Phase 4 はこの skill を全量投入で本格運用する。skill の使い勝手・取りこぼしは Phase 4 で洗い出し、必要なら
  改修する（`skill-creator`）。
