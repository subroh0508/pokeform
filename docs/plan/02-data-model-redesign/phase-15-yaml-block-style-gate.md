...
**M-A 全量投入（Phase 20）の手前**に置き、全186種という大量データが最初から block スタイルで入るようにして、
...
- スコープ外:
  ...
  - インデント幅・キー順・引用符など **block/flow 以外の YAML スタイル**（Biome / 既存慣習に委ね、本 Phase では
    flow 排除のみ）。
  - M-A 全量データの投入そのもの（**Phase 20**）。本 Phase は**仕組み（ゲート）+ 既存分の整形**に徹する。
...
- **Phase 20 への前提**: 本ゲート確定後、Phase 20（M-A 全量投入）は全186種を最初から block スタイルで投入し、
