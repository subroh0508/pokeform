...
排除する前段、が手順として明文化されていない。Phase 9（小データセット検証投入）と Phase 15（全量投入）が
...
- スコープ外:
  - データの実投入そのもの（Phase 9 = 3 種小データセット / Phase 15 = 全量投入が担う）。本 phase は **skill 改修のみ**。
...
- skill が materialize 方針を固定化することで、Phase 9 / Phase 15 が同じ手順で再現でき、レギュ更新（M-B 以降）でも
  routine 実行できる。
