...
全186種の全量投入（Phase 16）の**手前**に置き、構造データを旧スキーマ（raw 直読）で全量投入してからやり直す
...
- スコープ外:
  ...
  - M-A の全量データ投入（**Phase 16**）。本 Phase は**既存 catalog 分の移行 + 仕組み確定**のみ。
...
- **Phase 16 への前提**: 本 Phase 完了後、Phase 16（M-A 全量投入）は全186種の構造データを catalog YAML へ
