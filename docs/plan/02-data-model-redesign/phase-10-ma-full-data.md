...
- **Phase 9 申し送り（メガ先の技プール過大表現）**: `generate.ts` は mega 先のみの種族の `moves` を `catalog ∩ learnset`
  で導出するため（`scripts/generate.ts:340` 付近）、base 種族の per-reg `moves`（Serebii M-A subset）より**広くなる**
  ことがある（Phase 9 の `pokemon-data-reviewer` が charizard-mega 等で検出・non-blocking）。実ゲームでは技は base 形態に
  登録するため過大表現になる。**全量投入（約59メガ）前に、メガ先の `moves` を base の per-reg `moves` へ揃える設計**
  （`generate.ts` ロジック変更 or per-reg 追記）を確定し、不可逆なら `adr-new` で記録する。
