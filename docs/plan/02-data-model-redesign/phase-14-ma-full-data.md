# Phase 14 — M-A 全データ投入（全186種 + 全 movepool）
...
- スコープ内:
  ...
  - `catalog/species.yaml` に**全186種**追記（+ `megaLinks` 配列）、`catalog/moves.yaml` / `items.yaml` /
    `abilities.yaml` に追加 id を append-only 追記（技メタは Phase 12 で確定した SoT に従う）。`fetch:data` で
    構造データ（種族値・タイプ等）を `data/raw` 取得。
...
- **Phase 13（data/ ディレクトリ説明 README）完了**。`data/` 配下の索引が整備済みで、投入対象の各エントリの
  情報源・更新導線が `data/README.md` から辿れる。
...
- [ ] `fetch-pokeapi.ts` で追加 slug の**構造データ（種族値 / タイプ / 特性 / 持ち物 category）**を取得・`data/raw` キャッシュ（技威力等の技メタは Phase 12 の決定により raw から取らない）。
...
- 大量投入で取りこぼし・名称ゆれ・フォルム扱いの誤りが出やすい。`overrides.yaml` で構造データの世代差を補正する。
