...
次の Phase 10（全186種）を de-risk する。全件投入は**やらない**（スコープ外）。
...
- スコープ外:
  - **3 種以外の種族追加・全186種の全量投入**（Phase 10 が担う）。現ロスター26種（Phase 7）の moves 再投入もしない。
...
- なし（Phase 8 で定型化した skill を**使う**側・データ投入が中心）。投入中に手順の不備・取りこぼしが見つかれば
  `survey-regulation` を `skill-creator` で追従改修し、Phase 10 へ反映する（小データセットで skill を磨く狙い）。
...
- **de-risk が主眼**: 本 phase は Phase 10（全186種）の前に、本格スケールのパイプラインを 3 種で先に通すことで
