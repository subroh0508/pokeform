...
| `data-pipeline.md` | `scripts/**`<br>`data/**` | データ生成パイプライン（取得元 = pokemon-showdown 第一の正 / Serebii 速報 / PokeAPI ja 専任・SoT は構造 specs / 名前 languages / 解禁 per-reg の 3 軸直交・generated は raw 非依存合成の vendor 方式）。scripts/ や data/ を扱うとき適用する。 |
