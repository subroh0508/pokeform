...
scripts/        showdown/*（抽出・正）/ sync-showdown.ts / scrape-serebii.ts / sync-serebii.ts（速報）/ fetch-pokeapi.ts / materialize.ts（ja）/ generate.ts / gen-rules-index.ts
data/raw/       取得キャッシュ（PokeAPI ja + Serebii 速報・.gitignore）
...
- **取得元は vendor 方式 + 権威序列**: pokemon-showdown = 第一の正（構造 + 解禁）/ Serebii = 速報 / PokeAPI = ja 専任。取得 → 整形 → `src/generated/` をコミットし `data/raw` のみ gitignore。→ `data-pipeline.md` / ADR `0039`・`0040`
