...
data/champions/ 手動管理（rules/regulation/overrides.yaml・コミット）
...
- **検証は tsc のみ**（Zod 等の実行時バリデーション不採用）。不正は YAML/MD → codegen → `tsc --noEmit` で**型エラー**として弾く。→ `tsc-verification.md` / ADR `0002`
...
- **PokeAPI は vendor 方式**（取得 → 整形 → `data/generated/` をコミット）。`data/raw` のみ gitignore。→ `data-pipeline.md` / ADR `0004`
- **入力言語はファイル単位で `lang: ja|en` 宣言**。表示言語は `--lang`（独立）。問題検出時は**非0終了**。→ `cli-and-io.md` / ADR `0007`
...
- クロスエージェント方針: `.claude/rules/cross-agent.md` / ADR `0009`
