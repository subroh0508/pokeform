...
- 詳細は [`data-pipeline.md`](../.claude/rules/data-pipeline.md) / [ADR 0012](../docs/adr/0012-vendor-pokeapi-data.md)（vendor 方式）/ [ADR 0034](../docs/adr/archive/0034-move-meta-per-game-sot.md)（Serebii 第一優先・技メタ per-game SoT・ADR 0026 改訂）/ [ADR 0027](../docs/adr/0027-structural-data-catalog-sot.md)（catalog SoT・generate raw 非依存）。
...
| `champions/catalog/moves.yaml` | 技名 `id → { ja, en }`（名前 SoT・ゲーム非依存） | skill 著述（Serebii 第一優先・PokeAPI を信頼源にしない） | catalog | [`survey-regulation`](../.claude/skills/survey-regulation/SKILL.md) | [data-pipeline.md](../.claude/rules/data-pipeline.md) / [ADR 0034](../docs/adr/archive/0034-move-meta-per-game-sot.md) |
| `champions/regulations/champions/moves.yaml` | per-game 技メタ `id → { type / damageClass / power / accuracy / pp / priority }`（m-a/m-b 横断・Champions 固有値） | skill 著述（Serebii 第一優先・PokeAPI を信頼源にしない） | regulations（per-game） | [`survey-regulation`](../.claude/skills/survey-regulation/SKILL.md) | [data-pipeline.md](../.claude/rules/data-pipeline.md) / [ADR 0034](../docs/adr/archive/0034-move-meta-per-game-sot.md) |
...
| `champions/catalog/types.yaml` | 18 タイプ `id → { ja, en }` + 相性倍率 `damageTo`（非 1.0 のみ） | skill 著述 | catalog | skill/AI 経由 | [data-pipeline.md](../.claude/rules/data-pipeline.md) / [ADR 0025](../docs/adr/archive/0025-catalog-name-and-type-chart-sot.md) |
| `champions/regulations/<game>/<reg>.yaml` | 1 レギュ = 1 ファイル（ゲームでグルーピング・`champions/m-a.yaml` 等）。`name` / `period` / `items`（予約キー）+ 解禁種族キーごとの per-species `moves`（全量）・`mega[]`（block 記法） | skill 著述（Serebii 第一優先） | regulations（解禁の正本・per-reg 一本化） | [`survey-regulation`](../.claude/skills/survey-regulation/SKILL.md) で投入 → `check:regulation` で参照整合・schema 検証 | [data-pipeline.md](../.claude/rules/data-pipeline.md) / [ADR 0021](../docs/adr/0021-per-regulation-species-and-legality.md) / [ADR 0022](../docs/adr/0022-per-regulation-species-keyed-moves.md) |
...
| `generated/moves.ts` | 技名 dex（`moveDex` = id + name のみ） | catalog（派生） | `catalog/moves.yaml` | `generate:data` で再生成 | [data-pipeline.md](../.claude/rules/data-pipeline.md) |
| `generated/regulations/champions/moves.ts` | per-game 技メタ dex（`moveStatsDex` = type / damageClass / power / accuracy / pp / priority） | regulations（派生） | `regulations/champions/moves.yaml` | `generate:data` で再生成 | [data-pipeline.md](../.claude/rules/data-pipeline.md) / [ADR 0034](../docs/adr/archive/0034-move-meta-per-game-sot.md) |
