...
/**
 * 種族エントリの親型（統一パターンの XxxBase）。各 ID 参照（abilities/moves/items/types/
 * regulations）は対応する Dex のキーに統一し、詳細は `Dex[Id]` ルックアップで取る
 * （ID 単一ソース・[[type-conventions]]）。種族粒度は「種族値が一意 = 1 種族」。
 */
