...
export interface ItemBase {
  ...
  /** メガストーンの場合の**メガ形態** SpeciesId 群（X/Y を区別する・例 `["charizard-mega-x"]`）。
   *  `megaStoneFor` は base を指し X↔Y を引けないため、メガ形態種族の持ち物ロックに使う
   *  （generate が本リンクから per-reg メガ形態種の `items` 対応ストーンタプルを決定論導出する）。
   *  **gender メガのストーン（meowsticite）は 1 個で ♀♂両形態に対応するため配列**（`[<base>-female-mega,
   *  <base>-male-mega]`）・通常メガは 1 要素（ADR 0046）。per-reg dex 側で legality を見るため素の `string[]`
   *  （global SpeciesId への自己参照を避ける）。skill 著述 / PokeAPI 非由来。 */
  readonly megaSpecies?: readonly string[];
}
