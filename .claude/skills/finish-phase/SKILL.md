...
**(a) サブ README の phase チェック**: 対象フェーズが属する計画の README（ハーネスなら
`docs/plan/00-harness-setup/README.md`、MVP なら `docs/plan/01-mvp/README.md` 等）のフェーズ一覧で、
対象フェーズの行を `- [ ]` → `- [x]` に Edit で更新する（per-phase 進捗の SoT はサブ README）。
...
**(b) トップ `docs/plan/README.md` のロールアップ status**: その計画の**状況が変わったとき**（最初の
phase 着手で ⬜ 未着手 → 🚧 進行中、最終 phase 完了で 🚧 進行中 → ✅ 完了）だけ、トップ README の
「全体進捗（計画単位の status ロールアップ）」表の該当行の status を更新する。

- **status のみ**を更新する（n/n の分数は付けない）。
- **idempotent**: 既に同じ status なら触らない。phase 単位の進捗（n/n）でトップ表を更新しない
  （per-phase はサブ README が SoT・トップは計画単位の status のみ）。
...
- **ADR で決定を覆した / 大きく変えたフェーズでは、リポジトリ外の auto-memory**（`~/.claude/projects/<...>/memory/*.md`）に旧決定を前提とした記述が残っていないかも点検する。memory はリポジトリの SoT 更新に自動追従しないため、古い方針を保持し続け後続セッションが誤って recall する（実例: ADR 0026 で learnset 照合を撤去した後も関連 memory に旧記述が残存）。方針は [`adr.md`](../../rules/adr.md) の「決定を変えたら関連 memory の追従も点検する」節。
