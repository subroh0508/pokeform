...
`start-phase` 手順 1 と同じく `docs/roadmap/**/phase-*.md` を Glob で特定する。受け入れ基準 / 検証手順 /
...
- **(a) サブ README の phase チェック**: 対象計画の README（`docs/roadmap/<NN>-*/README.md`）のフェーズ一覧で、
  **対象フェーズの 1 行だけ**を `- [ ]`→`- [x]` に Edit する（per-phase 進捗の SoT）。他行・無関係記述に触れない。
  phase doc が役割分担表等の追記を**明示要求**するときのみ追記する（重複させない）。
- **(b) トップ `docs/roadmap/README.md` の status ロールアップ**: 計画の**状況が変わったときだけ**
  ...
  **🚧→✅ に上げる前に、サブ README の phase 一覧が全て `- [x]` であることを確認する**（(a) の per-phase 進捗と
  ...
  ✅ 化は全 `[x]` が前提）。
- **(c) 計画群の全 phase 完了時は `completed/` へ集約する**: ✅ 化した計画ディレクトリを
  `git mv docs/roadmap/<NN>-<slug> docs/roadmap/completed/<NN>-<slug>` で集約し、ロールアップ表のリンクを
  `completed/` パスへ更新する（active と完了を一目で分離）。**移動は plan ディレクトリが 1 階層深くなる rename**
  なので、inbound 参照追従（リポジトリ全体の `git grep`）と移動 doc 内の相対リンク深度補正を伴う。**移動 + 参照追従
  チェックリストの SoT は [[planning]] の「計画群完了時の `completed/` 集約」**（個別 phase 完了では動かさず、
  全完了時にまとめて移す）。この skill は移動を**促す**に留め、機械的追従は当該チェックリストに従う。
...
- フェーズ一覧 / 受け入れ基準: `docs/roadmap/completed/00-harness-setup/README.md`。
