...
> 日本語名 ja の取得元を PokeAPI names にする「なぜ」は [ADR 0032](../../../docs/adr/0032-japanese-name-source-pokeapi-names.md)。
...
（技メタを per-game regulations へ移した [ADR 0034](../../../docs/adr/0034-move-meta-per-game-sot.md) で境界が確定した）。
...
- **新規種族 / 持ち物の投入**（全種族投入 = `04-generated-layout-redesign` の全量投入 phase など）で catalog に未登録の id。
...
- 決定の「なぜ」: [ADR 0027](../../../docs/adr/0027-structural-data-catalog-sot.md)（構造データ SoT を catalog へ・`materialize` 新設）/
  ...
  [ADR 0034](../../../docs/adr/0034-move-meta-per-game-sot.md)（名前 = catalog / 技メタ = per-game regulations の境界）。
