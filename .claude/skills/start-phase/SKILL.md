...
phase doc には目的・前提（依存）・タスク・受け入れ基準・検証手順が書かれているが、着手時にこれを毎回
通読して要点を拾うのは抜けが出やすい。特に**依存フェーズが未マージ**だと手戻りになる。この skill は
着手前チェックを定型化し、依存の充足確認と受け入れ基準の事前把握を強制することで空振りを防ぐ。

## 入力

- **フェーズ識別子**（引数）: `8` / `phase-08` / `08-core-skills` / `Phase 8` のような表記を許容する。
  `01-mvp` 側の MVP フェーズも対象（`docs/plan/*/phase-*.md` を横断検索する）。
...
`docs/plan/**/phase-*.md` を Glob で列挙し、引数の番号 / slug に一致するものを選ぶ。番号は 2 桁ゼロ詰め
...
- 依存フェーズが参照するファイル（例: `pnpm verify`・`docs/plan/` 構造・`adr-new` skill）が実在するか
  を Glob / Read で確かめる。
...
- **依存の「実在」を確認する**: doc に「Phase 5 に依存」と書いてあっても、その成果物が worktree に
  無ければ着手できない。記述ではなく**ファイルの実在**で充足を判定する。
- **受け入れ基準を削らない**: 完了時に `finish-phase` がこの基準で照合する。要約しすぎて条件を落とさない。
- **実装まで踏み込まない**: この skill は「準備」。コード変更やファイル生成は着手後の別作業。
- **番号の正規化**: `8` と `phase-08` と `Phase 8` は同じ。ゼロ詰めの差で取りこぼさない。

## 関連

- 完了側: [`finish-phase`](../finish-phase/SKILL.md) — verify → 受け入れ基準照合 → 進捗更新。
- 検証ゲート: [`verify`](../verify/SKILL.md)。
- フェーズ一覧 / 役割分担 / 受け入れ基準: `docs/plan/00-harness-setup/README.md`（ハーネス計画）。
