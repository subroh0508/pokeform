...
**一括点検エントリ**。MVP の 2 コマンド（役割・手順を参照）を順に実行し、**機械判定可能な終了コード**と
指摘を 1 つの要約にまとめる。検証ロジックは CLI / domain に委譲し、本 skill は実行と要約に徹する
（機械ゲートを再実装しない・[[skill-authoring]]）。
...
- コマンド仕様: [[cli-and-io]]（CLI コマンド体系）/ 設計俯瞰は [individuals-and-parties](../../../docs/design/individuals-and-parties.md)。
