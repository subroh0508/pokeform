...
- **検証は tsc のみ**: 実行時バリデーション（Zod 等）を持ち込んでいないか。不正は YAML/MD → codegen →
  `tsc --noEmit` の**型エラー**で弾く設計か。→ [`tsc-verification`](../../../rules/tsc-verification.md) / ADR 0002。
- **型パターン**: `XxxBase` + `XxxDex` + `XxxId = keyof XxxDex`。種族粒度は「種族値が一意 = 1 種族」。英名 ID
  キー・`name.ja` 日本語名・逆引きマップ。→ [`type-conventions`](../../../rules/type-conventions.md) / ADR 0003。
...
- **データパイプライン**: PokeAPI は vendor 方式（取得 → 整形 → `data/generated/` をコミット、`raw` のみ
  gitignore）。**生成物への手編集混入**を疑う。→ [`data-pipeline`](../../../rules/data-pipeline.md) / ADR 0004。
- **CLI / IO**: 入力言語はファイル単位 `lang: ja|en` 宣言、表示は `--lang`（独立）。問題検出時は**非0終了**。
  外部入力の検証・エラー処理。→ [`cli-and-io`](../../../rules/cli-and-io.md) / ADR 0007。
