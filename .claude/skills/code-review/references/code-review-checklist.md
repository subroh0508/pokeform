...
- **型パターン**: `XxxBase` + `XxxDex` + `XxxId = keyof XxxDex`。種族粒度は「種族値が一意 = 1 種族」。英名 ID
  キー・`name.ja` 日本語名・逆引きマップ。→ [`type-conventions`](../../../rules/type-conventions.md) / ADR 0011。
...
- **データパイプライン**: PokeAPI は vendor 方式（取得 → 整形 → `data/generated/` をコミット、`raw` のみ
  gitignore）。**生成物への手編集混入**を疑う。**PokeAPI を Champions legality（使える技）/ 技メタ（威力等）の
  信頼源にしない**（Champions 非対応・ADR 0026）。技メタ SoT は catalog/`moves.yaml`、技の出自は Serebii 第一優先。
  `generate.ts` / `check:regulation` が `data/raw` learnset・raw 技メタへ回帰していないか疑う。
  → [`data-pipeline`](../../../rules/data-pipeline.md) / ADR 0012 / ADR 0026。
...
| paths | 重点観点 |
|---|---|
| `src/types/**`, `data/generated/**` | `XxxBase`+`XxxDex`+`XxxId=keyof XxxDex` パターン / 種族粒度 / **生成物への手編集混入** |
...
| `scripts/**`, `data/raw\|champions\|overrides` | data-pipeline 規約 / vendor 方針 / **秘匿情報のプレーンテキスト混入・スコープ** |
