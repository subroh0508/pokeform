...
- **正本変更時の全文同期 / doc-data 乖離**: 正本（rule / `src/` / catalog ヘッダコメント等）や用語を
...
- **coverage 除外ファイルのヘッダ / ドキュメントコメントの陳腐化**: skill rename・責務変更・機能拡張 PR で、
  **coverage 対象外のファイル（`src/codegen/*.ts` = `materialize.ts` 等）のファイル冒頭ヘッダ / 説明コメントが
  実態に追従しているか**を diff 外でも点検する。除外ファイルはテストが挙動を縛らないため、責務が変わっても
  ヘッダの旧記述（例「ja 専任」が実態 ja/en）が残りやすく機械ゲートで捕れない（#191 → #206 反復の除外死角）。
...
正本は各 rule（`.claude/rules/*`）と `src/`、設計俯瞰は [docs/design/](../../../../docs/design/README.md)。レビューでは**規約への
