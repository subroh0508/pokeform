...
`src/**` / `scripts/**` / `data/**` を変更した PR を**マージ前**に意味的にレビューし、重大度付きの指摘を返す
手順 skill。機械ゲート（型 / カバレッジ100% / Biome）で捕れない層 — **仕様忠実性・設計・安全性・整合性** — に
専念する。**指摘のみ・書込なし**（`allowed-tools` は read-only + git/gh 参照）。
...
[`code-review.md`](../../rules/code-review.md) の指摘フォーマットに従う:
...
各指摘に `blocking` / `non-blocking` / `nit` を付け、総括 → 重大度順で出力する。基準は
...
- **書き込まない**: この skill は指摘までが責務。修正コミットや auto-merge 実行はしない（auto-merge の
  発火条件は checklist の「auto-merge ゲート」節を参照）。
