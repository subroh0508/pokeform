...
description: >-
  ...
  （合計66 / 各≤32 制約・実現不能は報告）して現状 vs 目標・余剰の振り先候補を提案する。「ステータスを
  調整したい」「耐久ラインを逆算して」「素早さ○○抜きの振り方は?」「この技を確定耐えする配分は?」
  「火力と耐久どっちに振る?」「stat-tuning <path>」「実数値を見せて」と言われたとき、または個体の
  振り直しをブラッシュアップしたいときに使う。実数値計算・逆算ロジックは domain（calc-stats /
  damage / stat-tuning・テスト済み純関数）に委譲し、本 skill は確認・逆算・提案に徹する。個体の技 /
  特性 / 合計66 の妥当性検証は `author-individual`、パーティ全体の弱点 / 技範囲は `review-party` を使う。
...
- **現状を見る**: `pokeform stat <path>` で 6 実数値・性格補正記号・ポイント配分・耐久指数を表示する
  （`--lang ja|en`）。
- **逆算する**: 目標（素早さライン / 確定耐え）から `src/domain/stat-tuning.ts` の純関数で必要振りを
  求める。`solveOutspeed`（素早さ）/ `solveSurvival`（確定耐えのフロンティア）/ `buildTuningReport`
  （現状 vs 目標・余剰の振り先候補）。**実現不能なら `null`** をそのまま「無理（最大振りでも届かない）」と
  報告する。
