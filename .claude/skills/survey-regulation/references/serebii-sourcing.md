...
- **Champions 図鑑（種族ごとの使用可能技・種族値・タイプ・特性）**:
  ...
  - このページの **Standard / TM / Egg / Tutor などの技表を全て合算したものが「使用可能技 全件」**。Serebii の
    ...
    `regulations/champions/moves.yaml` へ記す（技名は `catalog/moves.yaml`・ADR 0034）。
- **解禁持ち物一覧**:
...
- **転記の役割分離**: `serebii-to-catalog` は Serebii 由来（技名 en は `catalog/moves.yaml`・**技メタは per-game
  `regulations/champions/moves.yaml`**・メガ先 / per-reg 解禁 / en）を書き、構造データ・
  日本語名 ja は `materialize`（PokeAPI vendor）に委ねる。**メガ linking は決定論で自動著述する**（base slug 既知 +
  メガ名の枝サフィックスから `<baseslug>-mega[-x|-y]` を導出し、`megaLinks` / メガ先種族エントリ / per-reg `mega[]` /
  メガストーンの `megaSpecies` を append/既存尊重で書く・[ADR 0033](../../../../docs/adr/0033-deterministic-mega-auto-authoring.md)）。
...
## 技メタ（type / power 等）は per-game `regulations/champions/moves.yaml` が SoT

技の type / damageClass / power / accuracy / pp / priority も **PokeAPI raw から導出しない**（ADR 0034 = ADR 0026
改訂）。**技の数値は Champions 固有値なので per-game の `regulations/champions/moves.yaml`（m-a/m-b 横断で共有）**
の各エントリへ Serebii 等で確定した技メタを hand-author する（技名 ja/en はゲーム非依存なので `catalog/moves.yaml`
が名前 SoT・Phase 11）。技メタの構造欠落は `generate.ts` の `satisfies MoveStats` が生成段の tsc で弾く。
...
6. **検証 / 再生成**: `check:regulation` 0 終了（参照整合 / schema・learnset 照合はしない・ADR 0026）→
   `generate:data`（raw 非依存・catalog のみ変換）→ `verify` 緑。
