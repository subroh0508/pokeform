...
- **Champions 図鑑（種族ごとの使用可能技・種族値・タイプ・特性）**:
  ...
  - このページの **Standard / TM / Egg / Tutor などの技表を全て合算したものが「使用可能技 全件」**。Serebii の
    ...
    技メタ（type / power / accuracy / pp / priority / damageClass）もこのページ等 Serebii で確定し per-game の
    `move-specs.yaml` へ記す（技名は `languages/moves.yaml`・ADR 0034 / 0035）。
- **解禁持ち物一覧**:
...
- **転記の役割分離**: `serebii-to-catalog` は Serebii 由来（技名 en は `languages/moves.yaml`・**技メタは per-game
  `move-specs.yaml`**・メガ先 / per-reg 解禁 / en）を書き、構造データ・
  日本語名 ja は `materialize`（PokeAPI vendor）に委ねる。**メガ linking は決定論で自動著述する**（base slug 既知 +
...
specs）**の各エントリへ Serebii 等で確定した技メタを hand-author する（技名 ja/en はゲーム非依存なので
`languages/moves.yaml` が名前 SoT・ADR 0035）。技メタの構造欠落は `generate.ts` の `satisfies MoveStats` が生成段の tsc で弾く。
...
3. **`serebii:catalog`**: 層1 中間 JSON から Serebii 由来（種族 / 全技 / 技メタ / メガ先 / per-reg 解禁 / en）を
   specs / per-reg / languages へ append / 既存尊重で転記。**メガ linking（`species-specs` の `megaEvolvesTo` /
   `mega-specs` の `baseSpecies` / per-reg `<reg>/mega.yaml` / メガストーンの `megaSpecies`）も決定論で自動著述**する
   （ADR 0033 / 0036）。`Mega ` 接頭の無い特殊形・未知 id は escalation（diagnostic）に出る。
4. **specs 更新チェックポイント → `update-catalog` へ委譲**: 構造データ（`dex` / `types` / `baseStats` /
   ...
   先に specs / languages を揃えてから手順 5 へ戻る。不足なし（0 終了）なら手順 5 へ。
5. **per-reg `mega` 確認 + per-reg 仕上げ**: メガ linking は手順 3 が自動著述済み（ADR 0033 / 0036）。`serebii:catalog` の
   **escalation diagnostic**（`Mega ` 接頭の無い特殊形・未知 id）が出た種だけ手当し、`<reg>/index.yaml` の `name` /
   `period` と `<reg>/items.yaml` を確認・補う。`escalated` 種は最終 WebFetch fallback で目視 + fixture 追加。
6. **検証 / 再生成**: `check:regulation` 0 終了（参照整合 / schema・learnset 照合はしない・ADR 0026）→
   `generate:data`（raw 非依存・specs / languages / per-reg を変換・合成）→ `verify` 緑。
