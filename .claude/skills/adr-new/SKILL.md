...
ADR は**不変ログ**なので、可変な plan ファイル（`docs/roadmap/**` の phase doc / 本文中の「Phase N」表記 / `OVERVIEW.md` リンク）を参照すると陳腐化する（方針は `.claude/rules/adr.md` の「参照のルール」）。生成した ADR 本文を grep で点検する:
