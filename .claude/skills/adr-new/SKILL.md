...
- 新規 ADR ファイル `docs/adr/NNNN-<slug>.md`（`template.md` から展開、Context/Decision/Consequences 記入済み、Status 設定済み）。
- `docs/adr/README.md` の ADR 一覧表に 1 行追記。
- supersede 時は、旧 ADR の Status 行のみ更新。

## 手順

### 1. 採番する

`ls docs/adr/` で既存の `NNNN-*.md` を列挙し、**最大連番 + 1** を新番号 `NNNN`（4 桁ゼロ詰め）とする。連番は飛ばさず・再利用しない。`README.md` / `template.md` は ADR ファイルではないので採番対象から除外する。
...
`docs/adr/template.md` を Read し、その構成（`# NNNN. <タイトル>` / Status / Date / Context / Decision / Consequences / 任意 Considered Options）に沿って新ファイルを Write する。見出し番号・タイトルを実際の値に置換し、Date は今日の日付（`YYYY-MM-DD`）にする。

### 4. 本文を対話で記入する

Context / Decision / Consequences を埋める。**ADR は「なぜ」を記録する場**なので、次の点を意識する（理由は `.claude/rules/adr.md`）:
...
- 新規の通常決定: 議論段階なら `Proposed`、その場で確定済みなら `Accepted`。
- Date を今日の日付にする。
...
2. 旧 ADR（`NNNN`）を Read し、**Status 行のみ**を `Superseded by ADR-MMMM` に更新する（本文は不変。補足が要れば末尾に追記のみ）。
3. `README.md` 一覧の旧 ADR の Status も `Superseded by ADR-MMMM` に更新する。
