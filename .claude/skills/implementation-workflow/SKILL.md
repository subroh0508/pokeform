...
本 skill は**実装を駆動する Generator**であり、各専門タスクは専任 skill に**委譲**する。機械ゲートや
レビュー観点を本 skill 内で**再実装しない**（[skill-authoring](../../rules/skill-authoring.md)）:

| 委譲先 | 役割 | フェーズ |
|---|---|---|
| [`start-phase`](../start-phase/SKILL.md) | 着手・計画読込 | 1 |
| [`verify`](../verify/SKILL.md) | 検証ゲート（`pnpm verify`） | 3 |
| [`code-review`](../code-review/SKILL.md) / [`harness-review`](../harness-review/SKILL.md) | 独立レビュー（Evaluator） | 6 |
| [`finish-phase`](../finish-phase/SKILL.md) | 完了・README 進捗更新・`adr-new` 促し | 8 |
| [`pr-retrospective`](../pr-retrospective/SKILL.md) | KPT learning 生成 | 8 |

**Generator / Evaluator 独立**: 実装（本 skill）と評価（レビュー skill）を分離し、Phase 6 は `Agent` で
レビュー skill を独立起動する（自己採点で代替しない）。
...
| 2 | 整合性チェック | 受け入れ基準と `architecture.md` / rules の整合・相互参照 dangling ゼロ | 整合レポート |
...
- **fix loop は上限 3**: 実装+verify（Phase 3）/ レビュー Critical 修正（Phase 6）は各々上限 3 回。
  累計超過で `blocked` 記録 + 人間通知して止める（無理に進めない）。
- **機械ゲート / レビュー観点を再実装しない**: 検証は `verify`、評価は `code-review` / `harness-review` に
  委譲。本 skill はそれらを**起動して束ねる**役（[skill-authoring](../../rules/skill-authoring.md)）。
- **worktree は Phase 0/9 ペア**: 作ったら必ず削除フェーズと対にする（残置・肥大化を防ぐ）。
  **未マージで強制削除しない**（`MERGED` 確認が前提）。
- **gh CLI / body-file 経由**: PR 操作は gh CLI、PR body / commit message は `/tmp` body-file。heredoc 直送・
  `--template` と `--body-file` 同時指定を避ける。
- **マージは auto-merge に委譲**: 独自のマージ規約（独自「auto-merge 禁止」や人間 approve 必須型）を
  導入しない。Phase 3 を見直す場合は rule と Phase 3 を同時更新する（[code-review](../../rules/code-review.md)）。
...
- phase doc: [`docs/plan/00-harness-setup/phase-11-implementation-workflow.md`](../../../docs/plan/00-harness-setup/phase-11-implementation-workflow.md)。
