...
description: >-
  ...
  引数は PR 番号 または 依存パッケージ名。マージは不可逆ゆえ〈可〉基準を全て満たす場合のみ実行する。
...
**メジャー含め最新追従**する方針（[ADR 0008](../../../docs/adr/0008-toolchain-version-source-of-truth.md)。版を固定せず、
...
- **入力**: **PR 番号**（例 `42`・その PR を直接対象）または **依存パッケージ名**（例 `vitest`・`gh pr list` で Dependabot PR を検索し複数なら一覧提示）。
- **出力（判定根拠）**: 更新パッケージと **from→to** / メジャー区分・破壊的変更の有無と**リポジトリ内使用箇所**（該当 / 非該当）・`pnpm verify` と `gh pr checks` の結果・**判定**（〈可〉/〈要人手〉）と実行アクション（マージ or エスカレーション）。
...
[references/examples.md](./references/examples.md) を参照。
...
- バージョン方針: [ADR 0008](../../../docs/adr/0008-toolchain-version-source-of-truth.md)（SoT=package.json/lockfile/.node-version・メジャー含め最新追従）。
