...
- **判定基準の SoT**: [`.claude/rules/harness-meta-criteria.md`](../../rules/harness-meta-criteria.md)。
...
- skill を改修する場合は **`skill-creator` skill を使う**（手書きで SKILL.md を起こさない。
  Phase 7 で新設される `skill-authoring.md` rule が SoT になる予定）。
- ADR が要る決定は **`adr-new` skill**（Phase 4）で起票する。
...
- [`harness-meta-criteria.md`](../../rules/harness-meta-criteria.md) に照らして各提案を分類:
  - **採用**: 反復 / 明確な Problem 解消 / 既存 rule・ADR と直接対応 / コスト<効果 / 人間が採用明示。
  - **見送り**: 後続フェーズ移行 / 重複 / コスト過大で現状運用可 / 人間が見送り明示。
  - **撤去**: 一定期間（既定 3 ヶ月）未参照 ＆ dangling 参照ゼロ ＆ 人間事前承認（**全充足**・2 段階必須）。
- **迷ったら見送り**を既定にする。
...
- **採用** → `harness/<purpose>` ブランチで改修 PR を作る:
  - `[rule]` … 対応する `.claude/rules/*.md` を Edit。
  - `[skill]` … `skill-creator` 経由で対象 `SKILL.md` を改修（canonical + `.agents/skills` symlink パリティ維持）。
  - `[template]` … PR テンプレ / learning template を Edit。
  - `[adr]` … `adr-new` でアーキ決定を ADR 化。
- **見送り** → 元 learning の `📝 harness-meta フィードバック > 見送り` に**理由を追記**（Edit）。
- **撤去** → **2 段階**（段階1: `status: removed` + cooldown / 段階2: cooldown 後に dangling ゼロ再確認して物理削除）。
  1 段階での即削除は禁止。
...
- **機械ゲートを再実装しない**: 品質ゲートは Git hooks / `pnpm verify`（Phase 5/9）の責務。
