...
- **ファイル rename**: 繰り下げる phase doc を `git mv phase-NN-*.md phase-(N+1)-*.md` し、本体見出し
  `# Phase N` を更新する。
- **番号整合の全走査**: `git grep -n "Phase N"`（旧番号）を計画ディレクトリ全体に打ち、**表・mermaid だけでなく
  散文 bullet 中の素の phase 番号**（「全量投入（N）」等）と**完了済み phase doc の forward 参照**まで全 hit を
  追従する。新番号が旧テーマを誤指ししていないか逆方向も確認する。
