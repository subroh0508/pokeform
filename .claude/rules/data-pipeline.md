...
- **`data/languages/`** = **コミット・skill 著述（人間直編集 NG）**。`{species,mega,items,moves,abilities,types}.yaml`（各 `id → { ja, en }`・**名前の SoT**・ゲーム非依存）＋ `regulations.yaml`（レギュ名・`id`（= `<game>-<reg>`・例 `champions-m-a`）→ `{ ja, en }`・PokeAPI に無く ja/en とも skill 著述・ADR 0035）。ja は `materialize` が PokeAPI `names` から補完、en は skill 著述（取得経路は下表・脚注）。`generate.ts` は raw を読まずこれを変換する。
