...
- **`data/languages/`** = **コミット・skill 著述（人間直編集 NG）**。`{species,mega,items,moves,abilities,types}.yaml`（各 `id → { ja, en }`・**名前の SoT**・ゲーム非依存）＋ `regulations.yaml`（レギュ名・`id`（= `<game>-<reg>`・例 `champions-m-a`）→ `{ ja, en }`・PokeAPI に無く ja/en とも skill 著述・ADR 0035）。日本語名 ja の取得元は PokeAPI `names`（ja-Hrkt 優先・ADR 0032）で `materialize` が補完、en は Serebii 表示名等で skill 著述。`generate.ts` は raw を読まずこれを変換する。
- **`src/generated/`** = **コミット**。`scripts/generate.ts` が specs / languages / per-reg を変換・合成して（raw 非依存・ADR 0027）Dex 単位の `.ts` を出力する:
  - `champions/{species,mega,item,ability,move,type}-specs.ts`（**構造 specs dex**・name 無し）。`species-specs`（`speciesSpecsDex`・dex/types/baseStats/abilities/megaEvolvesTo?）/ `mega-specs`（`megaSpecsDex`・dex/types/baseStats/ability/baseSpecies）/ `move-specs`（`moveSpecsDex`・**per-game 技メタ** type/damageClass/power/accuracy/pp/priority）/ `type-specs`（`typeSpecsDex`・`damageTo`）/ `item-specs`（`itemSpecsDex`・`category?`/`megaStoneFor?`/`megaSpecies?`）/ `ability-specs`（`abilitySpecsDex`・id のみ）。攻撃範囲分析（coverage）・ダメージ / 火力指数は `moveSpecsDex` を引く。
  ...
  - `champions/<reg>/{index,species,items,mega,species-moves}.ts` ＋ `champions/index.ts`（**per-reg・1 レギュ = 1 ディレクトリ**）= `index.ts` が `species-specs` ＋ `mega-specs` ＋ per-reg `species-moves` ＋ per-reg `mega` を**合成**して `PerRegSpecies` を満たす **per-reg 種族 dex `speciesDex`**（そのレギュの roster ∪ mega 先・per-reg 習得技 `moves` を含む legality の型正本）を作り、`RegulationBase` を満たすレギュメタ（`championsMA` 等）を export する。**per-reg 解禁メガを持つ base 種族の `items` は対応メガストーン id のタプル**で emit する（`item-specs` の `megaSpecies` リンクから決定論導出・対応ストーン欠落 / reg プール外は generate 側で fail-fast・例 `charizard.items: ["charizardite-x","charizardite-y"]`）。非メガ種は `items: "any"`（reg 解禁プール全件）。これにより `ItemNotHoldableBy<R,S,I>` がメガ可能種の対応ストーン以外を弾く（item legality・[[type-conventions]]）。集約 `champions/index.ts` が `regulationDex` に集める。**global 単一 `species.ts` は廃止**（統合 view へのフラット化は技プールが潰れ過剰許容になるため採らない・ADR `0021`）。実数値計算・名前表示・coverage はレギュ非依存のため `species-specs` / `mega-specs`（構造）＋ `languages`（名前）を引く（旧 `species-base.ts` 派生 base view は廃止・specs / languages 直交へ・ADR 0035）。
