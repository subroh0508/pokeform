...
- **species は canonical 明示 slug で書ける**（en 表記時）: variety を持つ種は明示 slug（`deoxys-normal` / `urshifu-single-strike` 等・PokeAPI 準拠 canonical）が id 正本（ADR 0044）。個体 / パーティ YAML は languages forward マップのキー（= canonical slug）で照合されるため、showdown bare 綴り（`deoxys`）ではなく canonical を書く。id 正本の定義は [[type-conventions]]、正規化層は [[data-pipeline]]。
