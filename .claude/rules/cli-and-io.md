...
- loader/codegen は宣言言語に従って全名称フィールドを解決し、**languages の forward マップ（`speciesNamesAll` / `moveNames` / `itemNames` 等・`id → { id, name }`）から実行時に逆引き（`reverseJa` / `reverseEn`）した名称マップ**で**正規 ID へ正規化**してから型生成・検証する（専用の生成逆引き `names.ts` は廃止・名前 SoT は languages・[[type-conventions]] / ADR 0035）。
