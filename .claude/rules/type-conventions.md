...
- **per-reg メガ可能種の items = 対応ストーンのみ**（item legality・[[data-pipeline]]）: `<reg>/index.ts` の `speciesDex` で、**per-reg 解禁メガを持つ base 種族の `items` は対応メガストーン id のタプル**（generate が `item-specs` の `megaSpecies` リンクから決定論導出・例 `charizard.items: ["charizardite-x","charizardite-y"]`）。非メガ種は `items: "any"`（reg 解禁プール全件）。`HoldableItems<R,S>` がタプルは Extract で、`"any"` は `RegulationItemId<R>`（メガ形態種は `MegaStoneOf<S>`）で解決し、メガ可能種が対応ストーン以外を持つと `ItemNotHoldableBy<R,S,I>` で弾く。
