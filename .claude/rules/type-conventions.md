...
- **メガストーン専有はメガ形態（メガシンカ後）種族のみ**（item legality・[[data-pipeline]]）: `<reg>/index.ts` の `speciesDex` は**全種 `items: "any"` を emit** する。`HoldableItems<R,S>` が `"any"` を per-reg プールへ接続し、**base（メガシンカ前）種族**は `RegulationItemId<R>`（解禁プール全件・メガストーン含む）へ、**メガ形態種族**（charizard-mega-x 等）は `MegaStoneOf<S>`（`item-specs` の `megaSpecies` リンクが当該形態を指すストーン 1 個）のみへ絞る。メガ形態種族が対応ストーン以外を持つと `ItemNotHoldableBy<R,S,I>` で弾く（`items` が `readonly ItemId[]` のタプルなら Extract 分岐で絞る潜在機構も残すが現状全種 `"any"`）。
