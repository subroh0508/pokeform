...
- **skill 新設 / 削除は symlink を一対で追従する**: 新設は canonical 作成 + `.agents/skills/<name>` symlink 追加、削除は canonical 削除 + symlink 削除を**同一 PR で**行う（片方だけ残すと dangling symlink / orphan 実体になる）。例: plan 10 で `survey-regulation` を削除（canonical + symlink）し `verify-showdown-pr` を新設（canonical + symlink）した。`ls .agents/skills/` の symlink 集合と `.claude/skills/` の実体集合が一致することを確認する。skill を削除したら **inbound 参照（rule / 他 skill / AGENTS / docs のリンク）も同一 PR で追従**し dangling を残さない。
